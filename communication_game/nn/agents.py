...
class Sender(BaseAgent):
    ...
    def forward(self, image, training=True):
        ...
        cumsum = tf.cumsum(tf.stack(symbol_mask, axis=0), axis=0)
        ...
        entropy = tf.reduce_mean(tf.transpose(tf.stack(entropy) * mask, (1, 0)), axis=1)
...
class Receiver(BaseAgent):
    ...
    def __init__(self, vocab_size, max_message_length, embed_dim, hidden_dim, vision_module,
                 activation='linear', n_distractors = 1, image_dim=32):
    ...
    def forward(self, message, images, training=True):
        ...
        image_embeddings = self.vision_to_hidden(self.vision_module(tf.reshape(images, (-1,self.image_dim,self.image_dim,3))))
