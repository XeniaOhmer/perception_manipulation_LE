...
parser.add_argument("--batch_size", type=int, default=128,
                    help="batch size")
parser.add_argument("--sim_sender",  type=str, default='default',
                    help="sender bias in {'default', 'color', 'shape', 'scale', 'all'}")
parser.add_argument("--sim_receiver",  type=str, default='default',
                    help="receiver bias in {'default', 'color', 'shape', 'scale', 'all'}")
parser.add_argument("--sf_sender", type=str, default='0-0',
                    help="smoothing factor for the sender vision module")
parser.add_argument("--sf_receiver", type=str, default='0-0',
                    help="smoothing factor for the receiver vision module")
...
parser.add_argument("--flexible_message_length", type=bool, default=False,
                    help="whether message length is flexible or fixed")
...
cnn_path_sender = all_cnn_paths[sim_sender + sf_sender]
cnn_path_receiver = all_cnn_paths[sim_receiver + sf_receiver]

cnn_sender = models.load_model(path_prefix + cnn_path_sender)
vision_module_sender = tf.keras.Model(inputs=cnn_sender.input,
                                      outputs=cnn_sender.get_layer(layer_name[representation_depth]).output)
                                                         
cnn_receiver = models.load_model(path_prefix + cnn_path_receiver)
vision_module_receiver = tf.keras.Model(inputs=cnn_receiver.input,
                                        outputs=cnn_receiver.get_layer(layer_name[representation_depth]).output)


###################################################
# initialize sender, receiver and training classes

sender = Sender(effective_vocab_size, message_length, embed_dim, hidden_dim, vision_module_sender,
                activation=activation, flexible_message_length=flexible_message_length)

receiver = Receiver(effective_vocab_size, message_length, embed_dim, hidden_dim, vision_module_receiver,
                    flexible_message_length=flexible_message_length, activation=activation,
                    n_distractors=n_distractors, image_dim=image_dim)
...
target_features = trainer.sender.vision_module(sender_in)
messages, logits, entropy, message_length, hidden_sender = trainer.sender.forward(sender_in, training=False)
    
rsa = RSA(sender, trainer.receiver)
...
groundedness = Groundedness(dataset, vocab_size, flexible_message_length=flexible_message_length)
...
with open(path + 'eval.pkl', 'wb') as handle:
    pickle.dump(eval_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)

# evaluation messages
message_dict = {}
sender_out, _, _, _, _ = trainer.sender.forward(sender_in, training=False)
indices = []
for i in range(n_classes):
...
if len(feature_dims) == 2:
    ...
    with open(path + 'message_dict.pkl', 'wb') as handle:
